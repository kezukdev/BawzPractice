...
public class PartyItems {
	...
	public PartyItems(final Player player) {
		...
		if (Practice.getInstance().getRegisterCollections().getProfile().get(player.getUniqueId()).getSubState().equals(SubState.NOTHING)) {
		...
		if (Practice.getInstance().getRegisterCollections().getProfile().get(player.getUniqueId()).getSubState().equals(SubState.QUEUE)) {
