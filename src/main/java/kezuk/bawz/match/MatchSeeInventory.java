...
public class MatchSeeInventory
{
    ...
    public MatchSeeInventory(final Player player) {
        this.content = player.getInventory().getContents();
        this.armor = player.getInventory().getArmorContents();
        this.hits = PlayerManager.getPlayers().get(player.getUniqueId()).getMatchStats().getHits();
        this.longestCombo = PlayerManager.getPlayers().get(player.getUniqueId()).getMatchStats().getLongestCombo();
        this.missedPots = PlayerManager.getPlayers().get(player.getUniqueId()).getMatchStats().getMissedPotions();
        this.thrownPotions = PlayerManager.getPlayers().get(player.getUniqueId()).getMatchStats().getThrownPotions();
        double remainLife = player.getHealth() / 2;
        NumberFormat nf = new DecimalFormat("0.#");
        this.life = nf.format(remainLife);
        this.food = (float) player.getFoodLevel();
        this.ownerName = player.getName();
        this.opponentName = Bukkit.getServer().getPlayer(Practice.getMatchs().get(PlayerManager.getPlayers().get(player.getUniqueId()).getMatchUUID()).getOpponent(player.getUniqueId())).getName();
        this.opponent = Practice.getMatchs().get(PlayerManager.getPlayers().get(player.getUniqueId()).getMatchUUID()).getOpponent(player.getUniqueId());
        ...
        this.ladder = Practice.getMatchs().get(PlayerManager.getPlayers().get(player.getUniqueId()).getMatchUUID()).getLadder();
        Practice.getInstance().getOfflineInventories().put(player.getUniqueId(), this);
    }
